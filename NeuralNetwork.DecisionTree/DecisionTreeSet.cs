...
namespace NeuralNetwork.DecisionTree
{
    public class DecisionTreeSet
    {
        ...
        public int NumberOfInstances => Instances.Count;

        public bool InstancesAreSameClass => Instances.Select(i => i.Output).DistinctBy(i => i.Value).Count() == 1;

        public DecisionTreeSet Split(Feature feature) => Split(feature.Axis, feature.Value);

        public DecisionTreeSet Split(string axis, string value) => new DecisionTreeSet
        {
            Instances = Instances.Select(i => i.Split(axis, value)).Where(i => i.Features.Any()).ToList()
        };
        ...
        private Tree SplitByAxis(string axis)
        {
            ...
            // split the set on each unique feauter value where the feature is
            // if of the right axis
            var splits = UniqueFeatures().Where(a => a.Axis == axis).Select(feature => new { splitFeature = feature, set = Split(feature) }).ToList();
        ...
        private Tree GroupByClass()
        {
            var groupings = Instances.DistinctBy(i => i.Output.Value)
                .ToDictionary(i => i.Features.First(),
                j => new Tree
                {
                    Leaf = j.Output
                });
